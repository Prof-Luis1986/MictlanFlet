...
def main(page: ft.Page):
    ...
    # Se agrega el audio del octavo nivel
    OctavoNivel = ft.Audio(
        src=os.path.join(audio_base_path, "Octavo_Nivel.mp3"), autoplay=False, volume=1, balance=0
    )
    page.overlay.append(OctavoNivel)
    
    page.title = "El Mictlan-Nivel 8"
    page.bgcolor = "#000000"
    page.fgcolor = "#FFFFFF"
    ...
    img = ft.Image(
        src=os.path.join(image_base_path, "Octavo-Nivel.png"),
        width=image_width, height=image_height, fit="contain"
    )
    
    btnPlay = ft.ElevatedButton("Da Click", on_click=lambda _: OctavoNivel.play())
