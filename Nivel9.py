...
def main(page: ft.Page):
    ...
    # Se agrega el audio del noveno nivel
    NovenoNivel = ft.Audio(
        src=os.path.join(audio_base_path, "Noveno_Nivel.mp3"), autoplay=False, volume=1, balance=0
    )
    page.overlay.append(NovenoNivel)
    
    page.title = "El Mictlan-Nivel 9"
    page.bgcolor = "#000000"
    page.fgcolor = "#FFFFFF"
    ...
    img = ft.Image(
        src=os.path.join(image_base_path, "Noveno-Nivel.jpeg"),
        width=image_width, height=image_height, fit="contain"
    )
    
    btnPlay = ft.ElevatedButton("Da Click", on_click=lambda _: NovenoNivel.play())
