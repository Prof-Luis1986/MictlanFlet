...
def main(page: ft.Page):
    ...
    # Se agrega el audio del cuarto nivel
    CuartoNivel = ft.Audio(
        src=os.path.join(audio_base_path, "Cuarto_Nivel.mp3"), autoplay=False, volume=1, balance=0
    )
    page.overlay.append(CuartoNivel)
    
    page.title = "El Mictlan-Nivel 4"
    page.bgcolor = "#000000"
    page.fgcolor = "#FFFFFF"
    ...
    img = ft.Image(
        src=os.path.join(image_base_path, "Cuarto-Nivel.jpeg"),
        width=image_width, height=image_height, fit="contain"
    )
    
    btnPlay = ft.ElevatedButton("Da Click", on_click=lambda _: CuartoNivel.play())
