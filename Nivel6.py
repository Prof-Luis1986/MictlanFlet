...
def main(page: ft.Page):
    ...
    # Se agrega el audio del sexto nivel
    SextoNivel = ft.Audio(
        src=os.path.join(audio_base_path, "Sexto_Nivel.mp3"), autoplay=False, volume=1, balance=0
    )
    page.overlay.append(SextoNivel)
    
    page.title = "El Mictlan-Nivel 6"
    page.bgcolor = "#000000"
    page.fgcolor = "#FFFFFF"
    ...
    img = ft.Image(
        src=os.path.join(image_base_path, "Sexto-Nivel.jpeg"),
        width=image_width, height=image_height, fit="contain"
    )
    
    btnPlay = ft.ElevatedButton("Da Click", on_click=lambda _: SextoNivel.play())
